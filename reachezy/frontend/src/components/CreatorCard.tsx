  ...
  return (
    <div className={`card flex flex-col border-l-4 transition-all duration-200 hover:border-primary-200 hover:shadow-md ${NICHE_ACCENTS[creator.niche] || 'border-l-gray-300'}`}>
      ...
      <div className="flex items-start gap-3">
        <div className={`flex h-12 w-12 shrink-0 items-center justify-center rounded-full overflow-hidden ${creator.profile_picture_url ? 'bg-white border border-slate-200' : 'bg-gradient-to-br from-primary-400 to-purple-500 text-lg font-bold text-white'}`}>
          {creator.profile_picture_url ? (
            <img src={creator.profile_picture_url} alt={creator.display_name || creator.username} className="h-full w-full object-cover" />
          ) : (
            initial
          )}
        </div>
      ...
      <div className="mt-4 flex gap-2">
        ...
        <a
          href={`/brand/messages?creator_id=${creator.creator_id}&name=${encodeURIComponent(creator.display_name || creator.username)}&username=${encodeURIComponent(creator.username)}&avatar=${encodeURIComponent(creator.profile_picture_url || '')}`}
          target="_blank"
          rel="noopener noreferrer"
